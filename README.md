...
All roughly a century old. **Only three of these ten currently have publicly-sourced, citable coordinates** — see Section 4 of CODING_AGENT_SPEC.md for the exact table. Do not invent coordinates for the rest. This is a running theme in this project: real, checkable specificity where we have it, honest gaps flagged where we don't, never a fabricated middle ground.
...
1. **Never invent MOIL production data, mine coordinates, or survey results.** Every place this is relevant is flagged `ASK USER` or `NEEDS_USER_INPUT` in the code and in CODING_AGENT_SPEC.md Section 2 and Section 4. If you hit one of these and no answer has been given yet, stop and ask — do not fill in a plausible-looking placeholder and move on silently.
...
4. **Validate reserve-mapping predictions with Leave-One-Out Cross-Validation, never in-sample-only.** With only 3-4 confirmed positive mine locations, checking whether those same locations rank highly *after training on them* is circular and will produce an artificially inflated, indefensible number. Always compute and report the honest LOOCV average, and label it as such wherever it's shown.

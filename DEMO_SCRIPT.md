...
2. **Zoom to Chikla, then Balaghat.** Point out that Chikla's displayed point is
   a government-corroborated point, not a lease-survey polygon. Use the
   validation card to explain that the displayed percentiles are computed
   leave-one-out evidence on four validated geometry records; six
   locality-only screening points are excluded, so this is not an in-sample
   score or geological certainty.
...
“This is a functional product preview with an auditable API contract. Four mine
locations have source-backed validation geometry and six remain screening-only
with recorded low confidence. The reserve cells and validation surface are
synthetic and illustrative. Scientific use begins only after authoritative
lease or Bhukosh geometry and model validation are confirmed.”
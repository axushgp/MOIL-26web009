...
4. **Validate reserve-mapping predictions with Leave-One-Out Cross-Validation, never in-sample-only.** The preview uses four validation-eligible geometries and reports the computed LOOCV average; six screening-only points remain map-only. A connected scientific run must use the strongest available mine geometries and still report the honest held-out result, never an in-sample score.
